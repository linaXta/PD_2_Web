...
function setupIndex() {
    ...
    // load JSON data
    fetch('http://localhost/data/get-top-movies')
        .then(
...
// setup single movie page
function setupSingle(id) {

    placeLoader();

    // load JSON data
    fetch('http://localhost/data/get-movie/' + id)
        .then(
            response => response.json()
        )
        .then(movie => {
            // on success remove spinner and render movie page
            removeLoader();
            renderSingle(movie);
        })
        .then(function(){
            // load related movies
            fetch('http://localhost/data/get-related-movies/' + id)
                .then(
                    response => response.json()
                )
                .then(movies => {
                    // render related movies links
                    renderRelated(movies);
                })
                .then(function(){
                    // setup link handling
                    setupLinks();
    ...
    for (const movie of movies) {
        ...
        // create info items
            ...
            if (movie.description && movie.description.length > 0) {
                ...
                lead.classList = 'lead';
                lead.textContent = (movie.description.split(' ').slice(0, 32).join(' ')) + '...';
...
// render main panel of single movie page
function renderSingle(movie) {
    ...
    // create info items
        ...
        if (movie.description && movie.description.length > 0) {
            let lead = document.createElement('p');
            lead.classList = 'lead';
            lead.textContent = movie.description;
            info.appendChild(lead);
        }
        ...
        dl.classList = 'row';
            ...
            priceValue.innerHTML = "&euro; " + movie.price;
            ...
            if (movie.genre.length > 0) {
                ...
                genreValue.textContent = movie.genre;
...
// render related movies panel of single movie page
function renderRelated(movies) {
    ...
    // create row element that will hold three related movies
    let row = document.createElement('div');
    row.classList = 'row mb-5';
